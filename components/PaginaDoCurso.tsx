...
// ─── Página do curso (usada pela rota pública e pela prévia) ──
export default async function PaginaDoCurso({
  slug,
  ignorarStatus = false,
}: {
  slug: string;
  /** Prévia: mostra o curso mesmo em rascunho, para revisão antes de publicar. */
  ...
  if (!course || (!ignorarStatus && course.status !== 'published')) {
    notFound();
  }
